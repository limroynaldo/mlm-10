...
public class ImportOrder extends SvrProcess
{
	...
	protected String doIt() throws java.lang.Exception
	{
		...
		sql = new StringBuilder ("UPDATE I_Order o ")
			  ...
			  .append(" WHERE o.Name=bp.Name AND o.AD_Client_ID=bp.AD_Client_ID) ")
			  .append("WHERE C_BPartner_ID IS NULL AND Name IS NOT NULL")
			  .append(" AND I_IsImported<>'Y'").append (clientCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		if (log.isLoggable(Level.FINE)) log.fine("Set BP from Name=" + no);
		
		sql = new StringBuilder ("UPDATE I_Order ")
				  .append("SET I_IsImported='E', I_ErrorMsg=I_ErrorMsg||'ERR=No BP, ' ")
				  .append("WHERE C_BPartner_ID IS NULL ")
				  .append(" AND I_IsImported<>'Y'").append (clientCheck);
			no = DB.executeUpdate(sql.toString(), get_TrxName());
			if (no != 0)
				log.warning ("No BP =" + no);
		...
		**/
		...
		//	AdSys Product
		sql = new StringBuilder ("UPDATE I_Order o ")
			  .append("SET AdSysProductID=(SELECT MAX(M_Product_ID) FROM M_Product p")
			  .append(" WHERE o.AdSysProductValue=p.Value AND o.AD_Client_ID=p.AD_Client_ID) ")
			  .append("WHERE AdSysProductID IS NULL AND AdSysProductValue IS NOT NULL")
			  .append(" AND I_IsImported<>'Y'").append (clientCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		if (log.isLoggable(Level.FINE)) log.fine("Set AdSysProduct from Value=" + no);		
		sql = new StringBuilder ("UPDATE I_Order ")
				  .append("SET I_IsImported='E', I_ErrorMsg=I_ErrorMsg||'ERR=Invalid Product, ' ")
				  .append("WHERE AdSysProductID IS NULL AND AdSysProductValue IS NOT NULL ")
				  .append(" AND I_IsImported<>'Y'").append (clientCheck);
		...
		try
		{
			...
			while (rs.next ())
			{
					...
					|| !oldDocumentNo.equals(cmpDocumentNo))
				{
					...
					// Set Trading Account
					int RDO_TradingAccount_ID = DB.getSQLValue(get_TrxName(), "SELECT MAX(RDO_TradingAccount_ID) FROM RDO_TradingAccount WHERE C_BPartner_ID=? AND RDO_MT_Username=?", 
							imp.getC_BPartner_ID(), imp.get_ValueAsString("RDO_MT_Username"));
					if (RDO_TradingAccount_ID>0)
						order.set_ValueOfColumn("RDO_TradingAccount_ID", RDO_TradingAccount_ID);
					order.set_ValueOfColumn("IsDepo", "Y");
					order.set_ValueOfColumn("IsDisclaimer", "Y");
				...
				if (imp.getQtyOrdered().compareTo(Env.ZERO) != 0)
				...
				if (order != null)
				{
					...
					order.saveEx();
					poreference = order.getDocumentNo();
				}
				
				/* AdSys Order */
				if (imp.get_ValueAsString("AdSysProductValue")!=null && 
						!imp.get_ValueAsString("AdSysProductValue").isEmpty() &&
						imp.get_ValueAsString("AdSysProductValue").compareTo("")!=0) {
					MOrder reforder = new MOrder (getCtx(), 0, get_TrxName());
					reforder.setClientOrg (imp.getAD_Client_ID(), imp.getAD_Org_ID());
					reforder.setC_DocTypeTarget_ID(imp.getC_DocType_ID());
					reforder.setIsSOTrx(imp.isSOTrx());
					if (imp.getDeliveryRule() != null ) {
						reforder.setDeliveryRule(imp.getDeliveryRule());
					}
					if (imp.getDocumentNo() != null)
						reforder.setDocumentNo(imp.getDocumentNo());
					//	Ship Partner
					reforder.setC_BPartner_ID(imp.getC_BPartner_ID());
					reforder.setC_BPartner_Location_ID(imp.getC_BPartner_Location_ID());
					if (imp.getAD_User_ID() != 0)
						reforder.setAD_User_ID(imp.getAD_User_ID());
					//	Bill Partner
					reforder.setBill_BPartner_ID(imp.getC_BPartner_ID());
					reforder.setBill_Location_ID(imp.getBillTo_ID());
					//
					if (imp.getDescription() != null)
						reforder.setDescription(imp.getDescription());
					reforder.setC_PaymentTerm_ID(imp.getC_PaymentTerm_ID());
					reforder.setM_PriceList_ID(imp.getM_PriceList_ID());
					reforder.setM_Warehouse_ID(imp.getM_Warehouse_ID());
					if (imp.getM_Shipper_ID() != 0)
						reforder.setM_Shipper_ID(imp.getM_Shipper_ID());
					//	SalesRep from Import or the person running the import
					if (imp.getSalesRep_ID() != 0)
						reforder.setSalesRep_ID(imp.getSalesRep_ID());
					if (reforder.getSalesRep_ID() == 0)
						reforder.setSalesRep_ID(getAD_User_ID());
					//
					if (imp.getAD_OrgTrx_ID() != 0)
						reforder.setAD_OrgTrx_ID(imp.getAD_OrgTrx_ID());
					if (imp.getC_Activity_ID() != 0)
						reforder.setC_Activity_ID(imp.getC_Activity_ID());
					if (imp.getC_Campaign_ID() != 0)
						reforder.setC_Campaign_ID(imp.getC_Campaign_ID());
					if (imp.getC_Project_ID() != 0)
						reforder.setC_Project_ID(imp.getC_Project_ID());
					//
					if (imp.getDateOrdered() != null)
						reforder.setDateOrdered(imp.getDateOrdered());
					if (imp.getDateAcct() != null)
						reforder.setDateAcct(imp.getDateAcct());
					
					// Set Order Source
					if (imp.getC_OrderSource() != null)
						reforder.setC_OrderSource_ID(imp.getC_OrderSource_ID());
					//
					
					reforder.setPOReference(poreference);
					reforder.set_ValueOfColumn("RDO_TradingAccount_ID", order.get_Value("RDO_TradingAccount_ID"));
					reforder.set_ValueOfColumn("IsDepo", "Y");
					reforder.set_ValueOfColumn("IsDisclaimer", "Y");				
					reforder.saveEx();
					noInsert++;
					lineNo = 10;				
					imp.set_ValueOfColumn("Ref_Order_ID", order.getC_Order_ID());				
					
					//	New OrderLine
					MOrderLine refline = new MOrderLine (reforder);
					refline.setLine(lineNo);
					lineNo += 10;
					if (imp.get_ValueAsInt("AdSysProductID") != 0)
						refline.setM_Product_ID(imp.get_ValueAsInt("AdSysProductID"), true);
					if (imp.getC_Charge_ID() != 0)
						refline.setC_Charge_ID(imp.getC_Charge_ID());
					if (imp.getQtyOrdered().compareTo(Env.ZERO) != 0)
						refline.setQty(imp.getQtyOrdered());
					else
						refline.setQty(Env.ONE);
					refline.setPrice();
					BigDecimal adsyspriceactual = (BigDecimal)imp.get_Value("AdSysPriceActual");
					refline.setPrice(adsyspriceactual);
					refline.setC_Tax_ID(1000000);
					if (imp.getFreightAmt() != null)
						refline.setFreightAmt(imp.getFreightAmt());
					if (imp.getLineDescription() != null)
						refline.setDescription(imp.getLineDescription());
					refline.saveEx();
					noInsertLine++;
				}
			}
